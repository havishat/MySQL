...
SELECT CONCAT(clients.first_name, ' ', clients.last_name) AS client_name, DATE_FORMAT(leads.registered_datetime, '%M') AS month_created,COUNT(leads.leads_id) AS num_leads
FROM clients
	LEFT JOIN sites ON clients.client_id = sites.client_id
	JOIN leads ON sites.site_id = leads.site_id -- no left join here
